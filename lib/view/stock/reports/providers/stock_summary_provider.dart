...
class StockSummaryServiceProvider extends ChangeNotifier {
  ...
  Future<StockReportModel> getStockSummary(String fromDate, String toDate, int? stItemGroupId) async {
    var client = http.Client();

    String rStItemGroupId = stItemGroupId != null ? '&StItemGroupId=$stItemGroupId' : '';

    var url = Uri.parse("${Strings.webShopUrl}${Strings.getStockSummary}?FromDate=$fromDate&ToDate=$toDate$rStItemGroupId");
    ...
    try {
      ...
      log(e.toString(), name: 'StockSummary Error');
    }
    notifyListeners();
  ...
  void selectStartDate(BuildContext context) async {
    ...
    if (selected != null && selected != selectedFromDate) {
      ...
      getStockSummary(selectedFromDate.toIso8601String().split("T")[0], selectedToDate.toIso8601String().split("T")[0], stItemGroupId ?? 0);
      notifyListeners();
    }
  }
